...
class DrawSomeInfo:
    def __init__(self):
        ...
        # self.yolov5_dete = torch.hub.load('./yolov5', 'custom', path = './weights/模型.pt', source = 'local')
    ...
    def generateThumb(self, raw_img, frame):
        ...
        if self.last_detect_res['detection'] == None:
            ...
            if len(results['boxes']) > 0:
                ...
                # label_id = results_np[0][5].astype(int)
                # label_name = results_np[0][6].astypr(string)
                ...
                # self.last_detect_res['detection'] = label_name
            else:
        ...
        # frame[y:y + h, x:x + w] = self.generateLabelArea(
        #     '{label_name}'.format(label_name = self.last_detect_res['detection']), x, y, w, h, frame)
    ...
    def recognize(self):
        ...
        with self.mp_hands.Hands(min_detection_confidence=0.7,
            ...
            while cap.isOpened():
                ...
                fpsTime = cTime
                self.image = drawInfo.cv2AddChineseText(self.image, "FPS: " + str(int(fps_text)), (10, 20),
                                                        textColor=(0, 255, 0), textSize=40)
                self.image = drawInfo.cv2AddChineseText(self.image, "Palm num: " + str(hand_num), (10, 60),
                                                        textColor=(0, 255, 0), textSize=40)
                self.image = drawInfo.cv2AddChineseText(self.image, "Pattern: " + str(drawInfo.hand_mode), (10, 100),
                                                        textColor=(0, 255, 0), textSize=40)
